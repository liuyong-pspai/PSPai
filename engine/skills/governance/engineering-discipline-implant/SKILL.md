...
从刘玉龙P07向伏羲超体的全栈对齐实战中提炼。不是"复制文件"——是**审计→量化差距→逐层对齐→双写同步→验证闭环**。

### 何时触发

父亲说「对照刘玉龙改造伏羲」「全面对齐」「给XX也植入」→ 执行本方法论。
...
### 陷阱8: 身份守护脚本禁止词拦截
某些 Agent（如伏羲）的网关启动脚本 `start_gateway.sh` 内含第三关「禁止词扫描」——检查 MEMORY.md 是否包含 `刘玉龙|Jarvis|贾维斯|哪吒` 等词。植入硬化清单时注意：MEMORY.md 不要出现目标守护脚本禁止的词。SOUL.md 中的家族排行（含「刘玉龙」）不受影响——守护脚本只扫描 MEMORY.md。
